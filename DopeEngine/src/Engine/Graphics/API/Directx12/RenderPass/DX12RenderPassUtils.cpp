#include "DX12RenderPassUtils.h"
#include <Engine/Core/Assert.h>


namespace DopeEngine
{
	D3D12_CULL_MODE DX12RenderPassUtils::get_dx12_cull_mode(const FaceCullMode mode)
	{
		switch (mode)
		{
	...
	D3D12_FILL_MODE DX12RenderPassUtils::get_dx12_fill_mode(const PolygonFillMode mode)
	{
		switch (mode)
		{
	...
	D3D12_PRIMITIVE_TOPOLOGY DX12RenderPassUtils::get_dx12_primitives(const PrimitiveTopology primitives)
	{
		switch (primitives)
		{
	...
	D3D12_PRIMITIVE_TOPOLOGY_TYPE DX12RenderPassUtils::get_dx12_primitive_type(const PrimitiveTopology primitives)
	{
		switch (primitives)
		{
			case DopeEngine::PrimitiveTopology::Points:
